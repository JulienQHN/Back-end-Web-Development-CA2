...
const dbroute =
  ...
  ` mongodb+srv://admin:admin@ca2.hn61b.mongodb.net/JulienQHN?retryWrites=true&w=majority`;
let db;
...
server.get("/api/sneakers/:id", (req, res) => {
  db.collection("sneakers").findOne(
...
server.delete("/api/sneakers", (req, res) => {
  db.collection("sneakers").deleteOne(
...
server.put("/api/sneakers", (req, res) => {
  ...
  db.collection("sneakers").updateOne(
    { _id: new ObjectID(id) },
    { $set: req.body },
    (err, result) => {
      if (err) throw err;
